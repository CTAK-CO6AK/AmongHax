...
public class RefuelEngineTask extends AutoTask {

	private int clickedID = -1;

	public RefuelEngineTask(int minDel, int maxDel) throws Exception {
		super(minDel, maxDel);
	}
	...
	public boolean tick(MinecraftClient client) {
		...
		if (clickedID != -1) {
			...
			for (int i = 1; i < 5; i++) {
				int slotID = i * 9 + 1;
				if (screen.slots.get(slotID).getStack().getItem().equals(Items.LAVA_BUCKET)) {
